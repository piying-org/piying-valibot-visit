...
describe('change-object', () => {
  ...
  it('部分修改', () => {
    let data = v.object({
      value1: v.string(),
      value2: v.number(),
      value3: v.pipe(v.boolean(), v.title('isBool')),
    });
    let changed = changeObject(data, {
      value1: (item) => {
        return v.pipe(item, v.title('hello'));
  ...
  it('intersect pipe', () => {
    let s1 = v.object({ v1: v.string() });
    let s2 = v.object({ v2: v.string() });
    let data = v.pipe(v.intersect([s1, s2]));
    let changed = changeObject(data, {
      v1: (item) => {
        return v.pipe(item, v.title('hello'));
      },
    });
    expect(changed.pipe[0].options).eq(changed.options);
    expect(v.getTitle(s1.entries.v1)).not.ok;
    expect(v.getTitle(changed.options[0].entries.v1)).eq('hello');
    expect(s2.entries.v2).ok;
    expect(v.getTitle(s2.entries.v2)).not.ok;
  ...
  it('object pipe', () => {
    let s1 = v.pipe(v.object({ v1: v.string() }));
    let s2 = v.pipe(v.object({ v2: v.string() }));
    let data = v.pipe(v.intersect([s1, s2]));
    let changed = changeObject(data, {
      v1: (item) => {
        return v.pipe(item, v.title('hello'));
    ...
    expect(changed.pipe[0].options).eq(changed.options);
    expect(changed.options[0].pipe[0].entries).eq(changed.options[0].entries);
    expect(changed.options[1].pipe[0].entries).eq(changed.options[1].entries);
    expect(v.getTitle(s1.entries.v1)).not.ok;
    expect(v.getTitle(changed.options[0].entries.v1)).eq('hello');
    expect(s2.entries.v2).ok;
    expect(v.getTitle(s2.entries.v2)).not.ok;
